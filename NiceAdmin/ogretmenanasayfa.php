...
try {
    ...
    if (isset($_SESSION['id']) && isset($_SESSION['role'])) {
        ...
        $sql = "SELECT * FROM t_class WHERE class_teacher_id = :teacherId";
        ...
        if ($teacherData) {
          ...
          $sql = "SELECT COUNT(*) AS student_count FROM t_classes_student WHERE class_id = :classId";
          $stmt = $conn->prepare($sql);
          $stmt->bindParam(":classId", $classId, PDO::PARAM_INT);
          $stmt->execute();
          $studentCountData = $stmt->fetch();
          
          $studentCount = $studentCountData['student_count'];
      } else {
          echo "Öğretmen bilgileri alınamadı.";
      }
      
    } else {
        echo "Öğretmen oturumu bulunamadı.";
    }
...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="ogretmenanasayfa.php">Ana Sayfa</a></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <div class="cardBox">
        <div class="card">
          <div>
            <div class="cardName">Sınıf Başarı Ortalaması</div>
            <div class="numbers"> 75.2 </div>
          </div>
        </div>
    </div> <br>
